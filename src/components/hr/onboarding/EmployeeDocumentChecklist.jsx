...
//   //  NEW: State for multiple payslip files
...
//         //  Handle multiple payslip URLs (parse JSON array)
//         if (checklistData.last_3months_payslip_url) {
//           try {
//             let payslipUrls = checklistData.last_3months_payslip_url;
//             console.log(payslipUrls)
//             // Remove extra quotes if present
//             if (typeof payslipUrls === 'string') {
//               payslipUrls = payslipUrls.replace(/^["']|["']$/g, '');
//               payslipUrls = JSON.parse(payslipUrls);
//             }
//             setExistingPayslips(Array.isArray(payslipUrls) ? payslipUrls : [payslipUrls]);
//           } catch (e) {
...
//   //  NEW: Handle multiple payslip file selection
//   const handlePayslipChange = (e) => {
//     const files = Array.from(e.target.files);
    
//     if (files.length > 3) {
//       alert('You can upload a maximum of 3 payslip files');
//       e.target.value = '';
//       return;
//     }
...
//     if (errors.last_3months_payslip) {
//       setErrors(prev => ({
//         ...prev,
...
//   //  NEW: Remove a specific existing payslip
//   const handleRemoveExistingPayslip = (index) => {
//     if (window.confirm('Are you sure you want to remove this payslip? This will be saved when you update.')) {
//       setExistingPayslips(prev => prev.filter((_, i) => i !== index));
//     }
//   };

//   //  NEW: Remove a newly selected payslip file
//   const handleRemoveNewPayslip = (index) => {
//     setPayslipFiles(prev => prev.filter((_, i) => i !== index));
//   };
...
//       // Add all file fields (except payslip)
//       const documentFields = [
//         'passport_size_photo', 'resume', 'signed_offer_letter', 'cheque_for_trainee',
//         'medical_certificate_for_trainee', 'pan_card', 'aadhaar_card', 'passport',
//         'tenth_marks_card', 'twelfth_marks_card', 'degree_marks_card_certificate',
...
//       //  Handle multiple payslip files
//       // if (payslipFiles.length > 0) {
//       //   // Add each payslip file with numbered naming
//       //   payslipFiles.forEach((file, index) => {
//       //     checklistData[`last_3months_payslip_${index + 1}`] = file;
//       //   });
//       // }
//       //  Handle multiple payslip files - Send all with same field name
//       if (payslipFiles.length > 0) {
//         // Multer expects multiple files with the same field name 'last_3months_payslip'
//         checklistData['last_3months_payslip'] = payslipFiles;
//       }
...
//         // Clear the payslip files after successful save
//         setPayslipFiles([]);
...
//   //  NEW: Render multiple payslip file input
//   const renderMultiplePayslipInput = () => {
//     return (
//       <div className="form-group md:col-span-2" key="last_3months_payslip">
//         <label className="block text-gray-700 font-medium mb-2">
//           Last 3 Months Payslip <span className="text-gray-500 text-sm">(Upload up to 3 files)</span>
//         </label>
...
//           disabled={isLoading}
//           className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60 text-sm ${
//             errors.last_3months_payslip ? 'border-red-500' : 'border-gray-300'
//           }`}
//         />
        
//         {/* Show existing payslips */}
//         {existingPayslips.length > 0 && payslipFiles.length === 0 && (
...
//                   <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
//                 </svg>
//                 Current Payslips ({existingPayslips.length})
//               </span>
//             </div>
            
//             {existingPayslips.map((payslip, index) => (
//               <div key={index} className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
//                 <div className="flex items-start justify-between mb-2">
//                   <span className="text-sm font-medium text-gray-700">Payslip {index + 1}</span>
//                   <button
//                     onClick={() => handleRemoveExistingPayslip(index)}
//                     disabled={isLoading}
//                     className="text-xs text-red-600 hover:text-red-800 font-medium disabled:opacity-50"
//                   >
//                     Remove
...
//                 New files selected ({payslipFiles.length})
//               </span>
//             </div>
            
//             {payslipFiles.map((file, index) => (
//               <div key={index} className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
//                 <div className="flex items-center justify-between">
//                   <div className="flex-1">
//                     <p className="text-sm font-medium text-blue-900">Payslip {index + 1}</p>
//                     <p className="text-xs text-blue-700">{file.name}</p>
//                   </div>
//                   <button
//                     onClick={() => handleRemoveNewPayslip(index)}
//                     disabled={isLoading}
...
//               <p className="text-xs text-blue-600 mt-2">
//                 ⓘ These will replace all existing payslips when saved
//               </p>
//             )}
//           </div>
//         )}
        
//         {errors.last_3months_payslip && (
//           <span className="text-red-500 text-sm mt-1 block">{errors.last_3months_payslip}</span>
//         )}
//       </div>
//     );
//   };
...
//         {/*  UPDATED: Multiple payslip input */}
//         {renderMultiplePayslipInput()}
...
//           💼 You can upload up to 3 payslip files at once
//         </p>
//       </div>
//     </div>
//   );
