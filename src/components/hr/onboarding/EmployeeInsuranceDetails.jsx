...
const EmployeeInsuranceDetails = ({ employeeId, isLoading, setIsLoading, onNextTab, onComplete }) => {
  ...
  const saveInsuranceDetails = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showModal(
          'Success',
          `Insurance details ${isUpdate ? 'updated' : 'added'} successfully!`,
          'success'
        );
        return true;
      } else {
        showModal('Error', response.message || 'Failed to save insurance details', 'error');
        return false;
      }
    } catch (error) {
  ...
  const handleDeleteInsuranceDetails = async () => {
    if (!insuranceDetails.insurance_id) {
      showModal('Error', 'Cannot delete unsaved insurance details', 'error');
      return;
    }

    showModal(
      'Confirm Delete',
      'Are you sure you want to delete these insurance details?',
      'warning',
      async () => {
        try {
          setIsLoading(true);
          ...
          if (response.success) {
            resetInsuranceDetails();
            showModal('Success', 'Insurance details deleted successfully!', 'success');
          } else {
            showModal('Error', response.message || 'Failed to delete insurance details', 'error');
          }
        } catch (error) {
          console.error('Error deleting insurance details:', error);
          showModal('Error', 'Failed to delete insurance details', 'error');
        } finally {
          setIsLoading(false);
        }
        closeModal();
